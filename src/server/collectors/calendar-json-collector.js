...
function detectPrefecture(source) {
  ...
  // URL-based detection
  for (const [domain, pref] of Object.entries(PREF_DOMAIN_MAP)) {
    if (domain === "tokyo") { if (/tokyo\.jp/.test(url)) return pref; }
    else if (new RegExp(domain + "\\.(jp|lg\\.jp)").test(url)) return pref;
  }
  // Key-prefix detection
  const km = key.match(PREF_KEY_RE);
  if (km) return PREF_DOMAIN_MAP[km[1]] || "東京都";
  // Legacy key fallbacks (keys without prefecture prefix)
  if (/kawaguchi|kasukabe|misato|okegawa|kazo|hanno|gyoda|honjo|hidaka|shiraoka|satte|iruma|fukaya|ogose|ogawa|yoshimi|kamikawa|namegawa/.test(key)) return "埼玉県";
  if (/narashino|kisarazu|isumi|sakura|sosa|sammu|shiroi|tohnosho|otaki|ichihara|sakae_chiba/.test(key)) return "千葉県";
  if (/maebashi|isesaki|fujioka_gunma/.test(key)) return "群馬県";
  if (/sano|nikko|moka|nasushiobara/.test(key)) return "栃木県";
  return "東京都";
}

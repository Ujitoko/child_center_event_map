...
function detectPrefecture(source) {
  ...
  if (/ageo|niiza|asaka|shiki|sakado|sugito|higashimatsuyama|yorii|kounosu/.test(key)) return "埼玉県";
  if (/yachiyo|asahi|kamogawa|katsuura|kimitsu|kyonan|yokoshibahikari/.test(key)) return "千葉県";
  if (/takasaki|ota_gunma|annaka|nakanojo/.test(key)) return "群馬県";
  if (/tochigi_city|yaita/.test(key)) return "栃木県";
  return "東京都";
}

...
/**
 * event_cal_multi カレンダー一覧ページをパース
 * カテゴリ <span> + <a> リンクを抽出
 */
...
function createCollectKashiwaEvents(deps) {
  const { geocodeForWard, resolveEventPoint, resolveEventAddress, getFacilityAddressFromMaster } = deps;
  const source = KASHIWA_SOURCE;
  const srcKey = `ward_${source.key}`;
  const label = source.label;

  return async function collectKashiwaEvents(maxDays) {
...
module.exports = { createCollectKashiwaEvents };

...
function createGetEvents(deps) {
  ...
  const [setagaya, ota, shinagawa, meguro, shibuya, minato, chiyoda, additional, hachioji, musashino, tachikawa, akishima, higashiyamato, kiyose, tama, inagi, hino, kokubunji, higashikurume, mitaka, kodaira, higashimurayama, kunitachi, ome, hamura, kawasaki, yokohama, sagamihara, ebina, kamakura, yokosuka, chigasaki, zama, zushi, yamato, hiratsuka, odawara, hadano, ayase, atsugi, isehara, minamiashigara, fujisawa, samukawa, aikawa, miura, oiso, hayama, nakai, kiyokawa, ninomiya, oi, yugawara, matsuda, manazuru, hakone, kaisei, yamakita, mizuho, okutama, hinode, hinohara, nagareyama, urayasu, noda, narashino, shiroi, kisarazu, isumi, tohnosho, otaki, funabashi, narita, chibaCity, kashiwa, yachiyo, asahi, kamogawa, yokoshibahikari, ichikawa, katsuura, kimitsu, kyonan, yotsukaido, matsudo, abiko, kamagaya, tomisato, shirako, kujukuri, yachimata, sodegaura, ichinomiya, choshi, sakura, futtsu, inzai, katori, togane, ichihara, sosa, sammu, sakaeChiba, mobara, tateyama, minamiboso, oamishirasato, shisui, kozaki, tako, shibayama, mutsuzawa, chosei, nagara, onjuku, chonan, naritaKosodate, abikoKosodate, kamagayaKosodate, sakuraLibrary, inzaiLibrary, kawaguchi, kasukabe, fujimino, misato, kawagoe, wako, warabi, ageo, niiza, asaka, toda, shiki, fujimi, sayama, yashio, saitamaCity, koshigaya, tokorozawa, kuki, kumagaya, kounosu, sakado, hanno, higashimatsuyama, gyoda, honjo, hidaka, shiraoka, satte, yorii, sugito, soka, tsurugashima, hasuda, iruma, kazo, fukaya, okegawa, ogose, ogawa, yoshimi, kamikawa, kamisato, yoshikawa, ogano, higashichichibu, kawajima, kitamoto, inaSaitama, yokoze, nagatoro, miyoshiSaitama, hatoyama, miyashiro, chichibu, namegawa, ranzan, matsubushi, minano, moroyama, hanyu, misatoSaitama, sano, nikko, moka, nasushiobara, tochigiCity, yaita, utsunomiya, ashikaga, kanuma, oyama, ohtawara, tochigiSakura, nasukarasuyama, shimotsuke, kaminokawa, mashiko, motegi, ichikai, haga, mibu, nogi, shioya, takanezawa, nasu, tochigiNakagawa, maebashi, isesaki, fujiokaGunma, takasaki, otaGunma, annaka, nakanojo, kiryu, numata, tatebayashi, shibukawaGunma, tomioka, midori, shinto, yoshioka, uenoGunma, kanna, shimonita, nanmoku, kanra, naganohara, tsumagoi, kusatsu, takayamaGunma, higashiagatsuma, katashina, kawaba, showaGunma, minakami, tamamura, itakura, meiwaGunma, chiyodaGunma, oizumi, ora, ...scheduleSupplements] = await batchCollect([
  ...
  const rawItems = [
    ...
    ...oizumi,
    ...ora,
    ...scheduleSupplements.filter(Array.isArray).flat(),
  ];
  ...
  const payload = {
    ...
    debug_counts: {
      raw: {
        ...
        city_narita_kosodate: naritaKosodate.length,
        city_abiko_kosodate: abikoKosodate.length,
        city_kamagaya_kosodate: kamagayaKosodate.length,
        city_sakura_library: sakuraLibrary.length,
        city_inzai_library: inzaiLibrary.length,
        ...
        city_tokorozawa: tokorozawa.length,
        city_kuki: kuki.length,
        city_kumagaya: kumagaya.length,
        city_kounosu: kounosu.length,
        city_sakado: sakado.length,
        city_hanno: hanno.length,
        city_higashimatsuyama: higashimatsuyama.length,
        city_gyoda: gyoda.length,
        city_honjo: honjo.length,
        city_hidaka: hidaka.length,
        city_shiraoka: shiraoka.length,
        ...
        city_iruma: iruma.length,
        city_kazo: kazo.length,
        city_fukaya: fukaya.length,
        city_okegawa: okegawa.length,
        town_ogose: ogose.length,
        town_ogawa: ogawa.length,
        town_yoshimi: yoshimi.length,
        town_kamikawa: kamikawa.length,
        town_kamisato: kamisato.length,
        city_yoshikawa: yoshikawa.length,
        town_ogano: ogano.length,
        village_higashichichibu: higashichichibu.length,
        town_kawajima: kawajima.length,
        city_kitamoto: kitamoto.length,
        town_ina_saitama: inaSaitama.length,
        town_yokoze: yokoze.length,
        town_nagatoro: nagatoro.length,
        town_miyoshi_saitama: miyoshiSaitama.length,
        town_hatoyama: hatoyama.length,
        town_miyashiro: miyashiro.length,
        city_chichibu: chichibu.length,
        town_namegawa: namegawa.length,
        town_ranzan: ranzan.length,
        town_matsubushi: matsubushi.length,
        town_minano: minano.length,
        town_moroyama: moroyama.length,
        city_hanyu: hanyu.length,
        town_misato_saitama: misatoSaitama.length,
        // Tochigi
        city_sano: sano.length,
        city_nikko: nikko.length,
        city_moka: moka.length,
        city_nasushiobara: nasushiobara.length,
        city_tochigi: tochigiCity.length,
        city_yaita: yaita.length,
        city_utsunomiya: utsunomiya.length,
        city_ashikaga: ashikaga.length,
        city_kanuma: kanuma.length,
        city_oyama: oyama.length,
        city_ohtawara: ohtawara.length,
        city_tochigi_sakura: tochigiSakura.length,
        city_nasukarasuyama: nasukarasuyama.length,
        city_shimotsuke: shimotsuke.length,
        town_kaminokawa: kaminokawa.length,
        town_mashiko: mashiko.length,
        town_motegi: motegi.length,
        town_ichikai: ichikai.length,
        town_haga: haga.length,
        town_mibu: mibu.length,
        town_nogi: nogi.length,
        town_shioya: shioya.length,
        town_takanezawa: takanezawa.length,
        town_nasu: nasu.length,
        town_tochigi_nakagawa: tochigiNakagawa.length,
        // Gunma
        city_maebashi: maebashi.length,
        city_isesaki: isesaki.length,
        city_fujioka_gunma: fujiokaGunma.length,
        city_takasaki: takasaki.length,
        city_ota_gunma: otaGunma.length,
        city_annaka: annaka.length,
        town_nakanojo: nakanojo.length,
        city_kiryu: kiryu.length,
        city_numata: numata.length,
        city_tatebayashi: tatebayashi.length,
        city_shibukawa: shibukawaGunma.length,
        city_tomioka: tomioka.length,
        city_midori: midori.length,
        vill_shinto: shinto.length,
        town_yoshioka: yoshioka.length,
        vill_ueno_gunma: uenoGunma.length,
        town_kanna: kanna.length,
        town_shimonita: shimonita.length,
        vill_nanmoku: nanmoku.length,
        town_kanra: kanra.length,
        town_naganohara: naganohara.length,
        vill_tsumagoi: tsumagoi.length,
        town_kusatsu: kusatsu.length,
        vill_takayama_gunma: takayamaGunma.length,
        town_higashiagatsuma: higashiagatsuma.length,
        vill_katashina: katashina.length,
        vill_kawaba: kawaba.length,
        vill_showa_gunma: showaGunma.length,
        town_minakami: minakami.length,
        town_tamamura: tamamura.length,
        town_itakura: itakura.length,
        town_meiwa: meiwaGunma.length,
        town_chiyoda_gunma: chiyodaGunma.length,
        town_oizumi: oizumi.length,
        town_ora: ora.length,
      },
    },
    items,
